...
1. 从 `knowledge_base/data/mapping_rules.json` 读取连续映射参数。
2. 根据七项体感改善量计算形态增量；环境干扰感按反向指标处理。
3. 从种子案例和历史人工反馈中检索相似体感变化，融合历史形态增量。
4. 有 LLM Key 时由 LangChain 把原图、规则结果、情景与 RAG 案例交给多模态 LLM 细化。
5. 无 LLM 或调用失败时保留本地规则/RAG结果。
6. 输出七项形态目标、前后增量、转换依据和引用案例，供专家编辑确认。
...
- RAG：本地 JSON 案例库先按七项体感改善向量与情景相似度召回，再把命中案例注入翻译官或制图员提示词。
- 兜底：未配置 Key、依赖不可用或调用失败时，仍使用本地映射参数与规则方案，便于离线联调。
...
种子案例不是专家实测值，也不表达因果关系，只用于验证数据协议、RAG 检索和前端联调。后续应以真实 VR 前后测、专家修正记录替换或校准。

示例体感基线对每个场景的 18 个历史情境评分取均值，并暂按 `naturalness←biophilia`、`relaxation←restoration`、`overall_impression←pleasure`、`environmental_disturbance←6-stimulation` 转换。最后一项只是为了适配“环境干扰感越低越好”的方向性占位，必须在获得正式量表数据后替换。
...
- 七项体感值不能被当作仅凭图片得到的真实 VR 实测值。若为联调补全，可由形态指标、情景要素、映射规则、RAG 案例和可选 LLM 生成模拟值，但必须标注为“模拟/估算”，不能与后续用户真实体验数据混用。
...
Task 2/3 默认使用阿里云百炼的 OpenAI-compatible 接口；未配置 Key 时仍走规则/RAG兜底，
