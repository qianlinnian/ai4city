...
CARTOGRAPHER_BASE_SYSTEM_PROMPT = (
    ...
    "object_actions 是唯一的最终执行清单：每一项都会原样转写到专家可编辑并发送给 Seedream 的最终方案；"
    "不得生成仅供审查、却不应执行的动作。动作超过5项时，应合并同一位置、同一对象类型的操作后再输出，"
    "而不是遗漏某项动作。quantity 应使用约数或范围表达可见设计约束（如‘3至5个，每个约1至2米’），"
    "不要把它表述为已测量的工程事实。专家意见中具有可见空间含义的内容必须转写为对象动作或保持不变区域；"
    "纯流程性说明可以保留在 rationale，不得伪装成图像编辑动作。"
